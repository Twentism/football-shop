...
def login(request):
    ...
    Expects POST fields `username` and `password`. Returns JSON with
    status and message.
    """
    # Keep behavior close to the original: only accept POST for credentials.
    if request.method != 'POST':
        return JsonResponse({"status": False, "message": "Method not allowed. Use POST."}, status=405)
